...
for ex =  51%SDGrecordingsA%GoodRecordings%GoodRecordingsPV%GoodRecordingsPV%selecN{1}(1,:) %1:size(data,1)
        ...
        for k = 1
            ...
            for u = eNeuron
                ...
                for t = 1:nT*mergeTrials
                    if dirStart ~= C(t,2)
                        yline(t/mergeTrials+0.5,'b',LineWidth=3);
                        dirStart = C(t,2);
                    end
                    if offStart ~= C(t,3)
                        yline(t/mergeTrials+0.5,'b',LineWidth=1);
                        offStart = C(t,3);
                    end
                ...
                cd(NP.recordingDir + "\Figs")
                if noEyeMoves
                    print(fig, sprintf('%s-MovBall-NoEyeMoves-%s-U%d-W%d-%dW-speed-500.png',NP.recordingName,orderNames{k},u,window_size(1),window_size(2)),'-dpng');

                else
                    print(fig, sprintf('%s-MovBall-%s-U%d-W%d-%dW-speed-500.png',NP.recordingName,orderNames{k},u,window_size(1),window_size(2)),'-dpng');
                end
...
%             directimesSorted = C(:,1)';
            ...
            for u = 1:length(respU)
                ...
                for f = 1:sizeX(4)
                    ...
                    spikeSum(selecInd,u,f) = sum(Mu(selecInd,1*j:min(f*msPerFarme,length(Mu))),2);
...
%             normMatrixSTD = reshape(std(Nb2),[1,1,1,nN])+eps;
            ...
            if noEyeMoves
                ...
                save(sprintf('RFuSelecTimeD-%s',NP.recordingName),'RFuSTDir','-v7.3')
