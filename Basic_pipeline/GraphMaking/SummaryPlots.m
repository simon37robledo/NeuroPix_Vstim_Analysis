...
%         RGsig = 0;
        ...
        allZscore = allZscore + mean(respNeuronsMB>sign);
...
%     DSI{i} = DSIi(goodNeurons);
    ...
    if MBsig
        MBc = [MBc ...
            zeros(1,length(ZscoreNeuronsMB(respNeuronsMB<=sign)))+ animal]; %%%Add animal number for colors
    end
    

end
%% Plot bar plot MB vs all neurons

fig = figure;bar([totalN,length(MBc)],'FaceColor','k','FaceAlpha',0.7)
ylabel('Number of neurons')
xticklabels({'Total neurons','Responsive to MB'})

fig.Position = [680   182   137   175];
cd('\\sil3\data\Large_scale_mapping_NP\Figs paper\1stFigure')
print(fig, 'barPlot.pdf', '-dpdf', '-r300', '-vector');


mean(MB)
allZscore = allZscore/numel(GoodRecordings);
