function [CM,CMP,h, cmapR_theta]=colormap2D(varargin)
parseObj = inputParser;
addParameter(parseObj,'nIntensities',100,@isnumeric);
addParameter(parseObj,'nColors',100,@isnumeric);
addParameter(parseObj,'blackAddition',0,@isnumeric); %adds black to top color, 0 value gives only white
addParameter(parseObj,'multiplicationFactor',2,@isnumeric);%[0-Inf] - makes rate of change stronger
addParameter(parseObj,'additionFactor',1,@isnumeric);%[0 1]- the higher makes changes weaker in total
...
if par.plotPolarExample
    ...
    for i=1:nR
        for j=1:nT
            ...
            polarplot(R(i)/nR*2*pi,Theta(j),'.','MarkerSize',10,'Color',  cmapR_theta( ceil(R(i)/nR*nRC), : , ceil(Theta(j)/nT*nTC) )  )
        end
    end
