...
for ex =1:size(data,1)
    ...
    [Mb] = BuildBurstMatrix(goodU,round(p.t/bin),round((stims-stimInter)/bin),round((stimInter*2+stimDur)/bin)); %Baseline matrix plus
...
%     end
    ...
    NeuronVals = cell(1,nN); %Each cell is a neuron, which has a matrix where the first column is maxVal of bins, 2nd and 3rd position of window in matrix...
    % 4th Z-score. 
...
%[1, 2, 3, 7, 8, 9, 10, 11, 12, 13, 17, 20, 22, 23, 24, 28, 32, 34, 36]
    ...
    for u =1:nN
        ...
        for i = 1:nT %Iterate across trials
            uM = squeeze(MrC(i,u,:))';%Create matrix per unique trial conditions

            max_position_Trial = zeros(nT,2); %Create 2 matrices, for mean inside max window, and for position of window, for each trial
            max_mean_value_Trial = zeros(1,nT);
            max_mean_value_Trial(i) = -Inf;
            ...
            for j = 1:size(uM, 2) - window_size(2) + 1 %Iterate across bins
                ...
                if mean_value >  max_mean_value_Trial(i)
                    max_mean_value_Trial(i) = mean_value;
                    max_position_Trial(i,:) = [i j];
                end

            end
            %Save across each trial (in a row) the max bin window
            %1%. Response
            NeuronRespProfile(i,1) = max_mean_value_Trial(i);

            %2%. WindowTrial
            NeuronRespProfile(i,2) = max_position_Trial(i,1);
            ...
            %according  to trial (d) the appropiate parameter > directions' offsets' sizes' speeds'
            NeuronRespProfile(i,5) = C(i,2);
            NeuronRespProfile(i,6) = C(i,3);
            NeuronRespProfile(i,7) = C(i,4);
            NeuronRespProfile(i,8) = C(i,5);
            NeuronRespProfile(i,9) = C(i,6);
...
xAxis = 1:cell2mat(ball.VSMetaData.allPropVal(64));%63 for pv103,64 for pv67 %65 PV139 and on   %:max(X,[],'all')-min(X,[],'all')-max(sizeV)/2;

yAxis = 1:cell2mat(ball.VSMetaData.allPropVal(64));%63 PV103,64 for pv67 %65 and on   %max(Y,[],'all')-min(Y,[],'all')-max(sizeV)/2;
...
centerXreal = cell2mat(ball.VSMetaData.allPropVal(62)); %PV139 63 %PV64 62 %PV103 61 
%% Convolution
...
%task: ask Mark about exact parameters of screen, what is the appopiate parameter? 

difX = max(ChangePosX,[],'all') - min(ChangePosX,[],'all');
...
coorRect = cell2mat(ball.VSMetaData.allPropVal(57)); %63 in PV139 and on

[x, y] = meshgrid(1:length(xAxis)+1, 1:length(yAxis)+1);
%[x, y] = meshgrid(1:1920+1,1:1920+1);
...
for s = 1:sizeN
    ...
    for i = 1:trialDivision*sizeN:length(ChangePosX)
        ...
        %matrix_nXnY = zeros(1920+1,1920+1);
