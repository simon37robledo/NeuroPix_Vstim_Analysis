...
if ~exist(sprintf('CorrectionAngle-%s.mat',NP.recordingName), 'file')
    ...
    CorrectionAngle = load(sprintf('CorrectionAngle-%s.mat',NP.recordingName)).CorrectionAngle;
...
theta = deg2rad(CorrectionAngle.angleCorrect);

% Create rotation matrix
R = [cos(theta), -sin(theta); sin(theta), cos(theta)];

% Calculate center of all ellipses
xc = meanCenter(1);
yc = meanCenter(2);

% Get all ellipse centers
ellipse_centers = [Data.center_x, Data.center_y];

% Get ellipse angles (already in radians)
ellipse_phi = Data.phi; 

% Initialize array for rotated centers
rotated_centers = zeros(size(ellipse_centers));

% Rotate each center point
for i = 1:size(ellipse_centers, 1)
    % Translate to origin
    translated = ellipse_centers(i,:) - [xc, yc];
    
    % Rotate point
    rotated = (R * translated')';
    
    % Translate back
    rotated_centers(i,:) = rotated + [xc, yc];
end

% Update the Data structure with rotated values
Data.center_x = rotated_centers(:,1);
Data.center_y = rotated_centers(:,2);

% Rotate ellipse angles (phi) - add theta directly since both are in radians
Data.phi = ellipse_phi + theta;
...
frame = rotateMatrix(frame,meanCenter,theta);
...
for gr = 1:GraphMultiplier %number of sets of panels (For x and y division, is 2)
...
%             trialIndex = [trialIndex zeros(1,length(frameTimes(frameTimes>=stimType(i,1) & frameTimes<stimType(i,2))))+i];
    ...
    elseif divideCenters >0
        ...
        scatter(meanCenter(1)-rectNew(1),meanCenter(2)-rectNew(2),10, 'filled','MarkerFaceColor','r')
        axis equal;
